...
/*   Updated: 2025/06/01 17:09:25 by arimanuk         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void flood_fill(char **map, t_map *map_s, int x, int y)
{
	...
	if (map[y][x] == '1' || map[y][x] == 'F' || map[y][x] == 'M')
		return;
	map[y][x] = 'F';
	flood_fill(map, map_s, x + 1, y); // right
	flood_fill(map, map_s, x - 1, y); // left
	flood_fill(map, map_s, x, y + 1); // down
	flood_fill(map, map_s, x, y - 1); // up
}
